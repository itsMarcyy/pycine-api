...
class MediaCreate(BaseModel):
    ...
    def normalize_type_midia(cls, value):
        if isinstance(
            value, str
        ):  # isinstance verifica se o valor é uma string, para evitar erros caso já seja do tipo MediaType
            return normalize_text(
                value
            )  # Chama a função de normalização para garantir que o valor seja consistente, independentemente de como o usuário o forneça (ex: "Filme", "filme", "FILME" serão tratados como "filme")
        return value
